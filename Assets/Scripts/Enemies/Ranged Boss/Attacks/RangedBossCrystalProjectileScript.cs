...
public class RangedBossCrystalProjectileScript : BaseRangedProjectileScript //Sebastian
{
    ...
    void Explode()
    {
        for(int i = -4; i < 5; i++)
        {
            for(int t = 0; t < 10; t++)
            {
	            GameObject crystalProj = Instantiate(crystalSpawn, this.transform.position + Vector3.Cross(new Vector3(1.5f * Mathf.Cos(t * 0.3f * Mathf.PI), 0, 1.5f * Mathf.Sin(t * 0.3f * Mathf.PI)), this.transform.up), this.transform.rotation);
