...
public class FireSlimeEnemy : WaterSlimeEnemy
{
    ...
    protected override void Split(GameObject temp)
	{
        if (generation < 1)
        {
            for (int i = 0; i < 4; i++)
            {
	            FireSlimeEnemy newSlime = spawner.GetComponent<SAIM>().SetSpawn(gameObject, this.transform.position + (this.transform.right * ((i * 2) - 1) * 2) + this.transform.up * 4).GetComponent<FireSlimeEnemy>();
